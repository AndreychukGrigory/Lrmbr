...
```
3. Запуск покрытия
- Копируем входные данные: Самые ценные файлы для покрытия это те, которые AFL нашел и сохранил в папке ```out/``` (в папках ```queue/``` и ```hangs/```), так как они исследовали максимальное количество уникальных путей.
